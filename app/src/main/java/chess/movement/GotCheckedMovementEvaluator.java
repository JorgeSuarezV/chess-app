...
public class GotCheckedMovementEvaluator
    extends AbstractEvaluator
     {

    @Override
    public Set<Move> isValidMove(Board board, Move move, Set<Move> moves)
        throws SelfCheckException, MovementException, OutOfBoundsException {
        ...
        PlayerColor oponentColor = playerColor == PlayerColor.WHITE
            ? PlayerColor.BLACK
            : PlayerColor.WHITE;
        ...
        for (Position actualPosition : actualPositions) { // searchForKing
            if (
                actualPosition.getPiece() != null &&
                actualPosition.getPiece().getPlayerColor() == playerColor &&
                actualPosition.getPiece().getType() == PieceType.KING
            ) {
        ...
        for (Position actualPosition : actualPositions) { // check all enemy Pieces
            if (
                actualPosition.getPiece() != null &&
                actualPosition.getPiece().getPlayerColor() == oponentColor
            ) {
                for (MovementEvaluator summativeMovementEvaluator : actualPosition
                    .getPiece()
                    .getMovementEvaluators()) { // check if threat
                    try {
                        if (
                            summativeMovementEvaluator.isThreatening(
                                board1,
                                new Move(
                        ...
                        if (
                            e.getClass() == SelfCheckException.class
                        ) throw new SelfCheckException();
                    }
                }
            }
