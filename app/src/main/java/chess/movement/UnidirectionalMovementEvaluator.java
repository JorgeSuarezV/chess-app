...
public class UnidirectionalMovementEvaluator
    ...
    private Position logicSetup(Board board, Move move)
        ...
        return board.getPosition(
            new Coordinate(xFrom + vectorX, yFrom + vectorY)
        );
    }
    ...
    public boolean isThreatening(Board board, Move move)
        ...
        Position positionToEvaluate = logicSetup(board, move);
        if (
            pathLogic(positionToEvaluate, move, board)
        ) return checkTargetMoveWithEvaluators(board, move);
        throw new OutOfBoundsException();
    }

    private boolean pathLogic(
        ...
        while (positionToEvaluate != null) {
            ...
            if (
                positionToEvaluate.getCoordinate().compareTo(move.getTo()) == 0
            ) {
                return true;
            } else if (
                pieceInTarget != null
            ) throw new PathBlockedException(); else positionToEvaluate =
                board.getPosition(
                    new Coordinate(
                        positionToEvaluate.getX() + vectorX,
                        positionToEvaluate.getY() + vectorY
                    )
                );
        }
        return false;
