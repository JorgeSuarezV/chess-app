...
public class MoveQuantityMovementEvaluator
    ...
    public Set<Move> isValidMove(Board board, Move move, Set<Move> moves)
        throws NotMovingAPieceException, MovementException {
        Position position = board.getPosition(move.getFrom());
        if (position == null) throw new NotMovingAPieceException(
        ...
        if (
            !(countMovements(board, move.getFrom(), piece) <= maxMove)
    ...
    public boolean isThreatening(Board board, Move move)
        throws MovementException, NotMovingAPieceException {
