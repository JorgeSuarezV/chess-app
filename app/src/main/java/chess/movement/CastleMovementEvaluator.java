...
public class CastleMovementEvaluator
    extends AbstractEvaluator
     {

    int yCoordinate;

    MovementEvaluator moveQuantityEvaluator = new MoveQuantityMovementEvaluator(
        0
    );
    ...
    public Set<Move> isValidMove(Board board, Move move, Set<Move> moves)
        ...
        if (
            move.getTo().getY() != move.getFrom().getY() ||
            move.getFrom().getY() != yCoordinate
        ) throw new MovementException(); // que este en el inicio en y
        ...
        if (xMove == -2) { // esta para la izquierda
            ...
            Position rookTo = board.getPosition(
                new Coordinate(
                    fromPosition.getCoordinate().getX() - 1,
                    yCoordinate
                )
            );
            Position position = board.getPosition(rookFrom);
            if (
                rookTo != null && rookTo.getPiece() != null
            ) throw new MovementException();
            if (
                position != null &&
                position.getPiece().getType() == PieceType.ROOK
            ) { // hay un rook en la izquierda
                Move rookMove = new Move(
                    rookFrom,
                    new Coordinate(move.getFrom().getX() - 1, yCoordinate)
                );
                moveQuantityEvaluator.isValidMove(board, rookMove, moves);
                checkMovementEvaluators(
                    board,
                    rookMove,
                    moves,
                    position.getPiece()
            ...
            Coordinate rookFrom = new Coordinate(
                board.getWidth() - 1,
                yCoordinate
            );
            Position rookTo = board.getPosition(
