...
public class TakeMovementEvaluator extends AbstractEvaluator implements MovementEvaluator{
    ...
    @Override
    public Set<Move> isValidMove(Board board, Move move, Set<Move> moves) throws OutOfBoundsException, CannotTakePieceException, MovementException {
        ...
        if (position1.getPiece() != null && position2.getPiece() != null && position2.getPiece().isTakeable(position1.getPiece())) {
...
//            return moves;
    ...
    @Override
    public boolean isThreatening(Board board, Move move) throws OutOfBoundsException, CannotTakePieceException, MovementException {
        Position position1 = board.getPosition(move.getFrom());
        Position position2 = board.getPosition(move.getTo());
        if (position1 == null || position2 == null) throw new OutOfBoundsException();
        if (!canTake && position2.getPiece() == null) {
        ...
        if (position1.getPiece() != null && position2.getPiece() != null && position2.getPiece().getPlayerColor() != position1.getPiece().getPlayerColor()) {
            if (!canTake) throw new MovementException();
            return true;
        }
        else throw new CannotTakePieceException();
