...
public class MovementEvaluatorSetFactory {
    ...
    public static Set<MovementEvaluator> createMusketeerMovementEvaluator() {
        Set<MovementEvaluator> evaluators = new HashSet<>();
        evaluators.add(
            new RangeMovementEvaluator(1, 2, Set.of(PieceType.PAWN))
        );
    ...
    public static Set<MovementEvaluator> createKingMovementEvaluators(
        PlayerColor color,
        int boardSize
    ) {
        Set<MovementEvaluator> evaluators = createQueenMovementEvaluators();
        if (color == PlayerColor.WHITE) evaluators.add(
            new CastleMovementEvaluator(0)
        ); else evaluators.add(new CastleMovementEvaluator(boardSize - 1));
        for (MovementEvaluator movementEvaluator : evaluators) {
            movementEvaluator.addMovementEvaluator(
                new LimitMovementEvaluator(1)
            );
        }
        return evaluators;
    }
    ...
    public static Set<MovementEvaluator> createPawnMovementEvaluators(
        PlayerColor color
    ) {
        int vectorY = 1;
        if (color == PlayerColor.BLACK) vectorY = -1;
        MovementEvaluator evaluator1 = new UnidirectionalMovementEvaluator(
            1,
            vectorY
        )
            .addMovementEvaluator(new LimitMovementEvaluator(1))
            .addMovementEvaluator(new TakeMovementEvaluator(true));
        MovementEvaluator evaluator2 = new UnidirectionalMovementEvaluator(
            -1,
            vectorY
        )
            .addMovementEvaluator(new LimitMovementEvaluator(1))
            .addMovementEvaluator(new TakeMovementEvaluator(true));
        MovementEvaluator evaluator3 = new UnidirectionalMovementEvaluator(
            0,
            vectorY
        )
        ...
        MovementEvaluator evaluator4 = new UnidirectionalMovementEvaluator(
            0,
            vectorY
        )
            .addMovementEvaluator(new TakeMovementEvaluator(false))
        ...
        return new HashSet<>(
            Arrays.asList(evaluator1, evaluator2, evaluator3, evaluator4)
        );
    }
