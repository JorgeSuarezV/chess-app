...
public abstract class AbstractEvaluator implements MovementEvaluator{
    ...
    public Set<Move> reachedPosition(Board board, Move move, Set<Move> moves) throws OutOfBoundsException, NotMovingAPieceException, MovementException, CannotTakePieceException, PathBlockedException, SelfCheckException {
        Position toPosition = board.getPosition(move.getTo());
        Position fromPosition = board.getPosition(move.getFrom());
        if (toPosition == null || fromPosition == null) throw new OutOfBoundsException();
    ...
    public Set<Move> checkValidMoveWithEvaluators(Board board, Move move, Set<Move> moves) throws NotMovingAPieceException, MovementException, PathBlockedException, OutOfBoundsException, CannotTakePieceException, SelfCheckException {
        for (MovementEvaluator movementEvaluator : movementEvaluators) {
            boolean result = movementEvaluator.isValidMove(board, move, moves).size() != 0;
