...
public class WinValidator implements Validator{
    ...
    public MoveResult checkMove(Board board, Move move, PlayerColor currentPlayerColor) {
        ...
        PlayerColor oponentColor = currentPlayerColor == PlayerColor.WHITE ? PlayerColor.BLACK : PlayerColor.WHITE;

        for (Position actualPosition : actualPositions) {
            Piece piece = actualPosition.getPiece();
            if (piece == null || piece.getPlayerColor() == currentPlayerColor) continue;
            for (Position toPosition : actualPositions) {
                Move move1 = new Move(actualPosition.getCoordinate(), toPosition.getCoordinate());
                try{
